...
def read_circle(path: str) -> tuple[str, str]:
    with open(path, 'r', encoding='utf-8') as file:
        coord, radius = (file.readline().strip() for _ in range(2))
        return coord, radius


def read_dots(path: str) -> list[str]:
    with open(path, 'r', encoding='utf-8') as file:
        dot_list = [row.strip() for row in file.readlines()]
        return dot_list


def dot_check(c_x: Fraction, c_y: Fraction, c_radius: Fraction, d_x: Fraction, d_y: Fraction) -> int:
    segment = ((d_x - c_x) ** 2 + (d_y - c_y) ** 2) ** 0.5
    return 0 if segment == c_radius else 2 if segment > c_radius else 1


def print_dots_check(path_circle: str, path_dots: str) -> None:
